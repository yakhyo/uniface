...
- AdaFace `IR_50` weights (WebFace4M, 166 MB): 95.44% IJB-B, 96.98% IJB-C.
  More accurate than `IR_18` at 1.8x the size. `IR_18` remains the default.

### Fixed

- Model licence table no longer implies the MIT licence covers pretrained
  weights: code and weights licences are now separate columns. InsightFace
  weights (SCRFD, ArcFace, AgeGender, Landmark106) are non-commercial upstream,
  EdgeFace is BSD-3-Clause rather than MIT, and AgeGender, Landmark106 and
  EdgeFace were missing from the table entirely.
